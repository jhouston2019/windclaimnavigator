// Enhanced Document Generator Integration
// This file provides the new Supabase-integrated document generation

import { handleGenerateAndSave } from './generatorHandler.js';
...
export class EnhancedDocumentGenerator {
  ...
  getCurrentLocale() {
    ...
    const languageToggle = document.querySelector('input[name="language"]:checked');
  ...
  showResult(result) {
    ...
    const saveToDocuments = document.getElementById('saveToDocuments');
   ...
   */
  async handleFormSubmitLegacy(data) {
    try {
      this.showLoading(true);
      this.hideResultPanel();
...
// Make available globally for HTML integration
window.EnhancedDocumentGenerator = EnhancedDocumentGenerator;
