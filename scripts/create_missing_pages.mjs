import {write, exist} from './_helpers.mjs';

const top = ['advisory','maximize','tactics','state-rights','trackers','evidence','settlement','negotiation','litigation','calculator','marketplace'];
const topTitle = {
  'advisory':'🧭 Situational Advisory','maximize':'📈 Maximize Your Claim','tactics':'🧠 Insurance Company Tactics',
  'state-rights':'⚖️ State Rights & Deadlines','trackers':'🗺️ Trackers & Timeline','evidence':'📸 Evidence Organizer',
  'settlement':'⚖️ Settlement Comparison','negotiation':'💬 Negotiation Scripts','litigation':'🚀 Escalation Readiness',
  'calculator':'💵 Financial Impact Calculator','marketplace':'🏪 Professional Marketplace',
};
const docSlugs = ['appeal-letter','demand-letter','damage-inventory','claim-timeline','repair-vs-replace','expenses-log','appraisal-demand','delay-complaint','coverage-clarification'];
const docTitle = {
  'appeal-letter':'Appeal Letter','demand-letter':'Demand Letter','damage-inventory':'Damage Inventory Sheet',
  'claim-timeline':'Claim Timeline / Diary','repair-vs-replace':'Repair or Replacement Cost Worksheet',
  'expenses-log':'Out-of-Pocket Expense Log','appraisal-demand':'Appraisal Demand Letter',
  'delay-complaint':'Notice of Delay Complaint','coverage-clarification':'Coverage Clarification Request',
};
const analysis = [
...
const topTpl = (title)=>`<!doctype html><html lang="en"><head>
...
<main class="container"><div class="card"><h1 class="section-h">${title}</h1><p class="lead">Page scaffold.</p></div></main>
...
<main class="container"><div class="card"><h1 class="section-h">${title}</h1>
  <div class="card">
...
<script type="module">
import { callAI, createDoc } from '/app/assets/js/api-client.js'; import { qs,on } from '/app/assets/js/ui-helpers.js';
const slug='${slug}';
...
<main class="container"><div class="card"><h1 class="section-h">${title}</h1><p class="lead">${desc}</p>
  <div class="card">
    <label>Input</label>
...
<script type="module">
import { analyzeClaim, createDoc } from '/app/assets/js/api-client.js'; import { qs,on } from '/app/assets/js/ui-helpers.js';
...
on(qs('#run'),'click', async ()=>{
  ...
  try{ const res=await analyzeClaim({analysisType,text:v}); const html=res.assessment||res.comparison||res.report||res.analysis||JSON.stringify(res);
    qs('#tool-output').innerHTML=html||'(no output)'; }catch(e){ qs('#tool-output').innerHTML=\`<div style="color:#dc2626">Error: \${e.message}</div>\`; }
});
...
for (const k of top) {
  const f=`app/${k}/index.html`;
  if (!exist(f)) { write(f, topTpl(topTitle[k]||k)); created.push(f); }
}
for (const s of docSlugs) {
  const f=`app/documents/${s}.html`;
  if (!exist(f)) { write(f, docTpl(s, (docTitle[s]||s))); created.push(f); }
}
// Ensure CA index exists
const caIndex = 'app/response-center/claim-analysis-tools/index.html';
if (!exist(caIndex)) {
  write(caIndex, `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
  ...
  <main class="container"><section class="grid tools tools-xl">
  ...
  <script type="module" src="/app/assets/js/diagnostics.js"></script></body></html>`);
  created.push(caIndex);
}
for (const [slug,title,desc,type] of analysis) {
  const f=`app/response-center/claim-analysis-tools/${slug}.html`;
  if (!exist(f)) { write(f, analysisTpl(slug,title,desc,type)); created.push(f); }
}

import {writeFileSync} from 'fs';
writeFileSync('_build/created.json', JSON.stringify({created},null,2));
console.log('✅ Created', created.length, 'missing pages under /app');
